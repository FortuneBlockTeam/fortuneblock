...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/FortuneblockParams"
else
    PARAMS_DIR="$HOME/.fortuneblock-params"
fi
...
DOWNLOAD_URL="hhttps://github.com/FortuneBlockTeam/fortuneblock/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with FTB_DISABLE_SOMETHING=1
FTB_DISABLE_WGET="${FTB_DISABLE_WGET:-}"
#FTB_DISABLE_IPFS="${FTB_DISABLE_IPFS:-}"
FTB_DISABLE_CURL="${FTB_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$FTB_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$FTB_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$FTB_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Fortuneblock parameters!
...
This script will fetch the Fortuneblock SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Fortuneblock SNARK parameters. Note that it is
