...
*Setup instructions for a Gitian build of Fortuneblock Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fortuneblock
...
- [Building Fortuneblock Core](#building-fortuneblock-core)
...
Clone the git repositories for Fortuneblock Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ftbfun/fortuneblock
git clone https://github.com/ftbfun/gitian.sigs.git
```
...
in the Fortuneblock Core repository under 'Fetch and create inputs' to install sources which require
...
To build Fortuneblock Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Fortuneblock Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fortuneblock/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ftbfun/fortuneblock
...
For example:
```bash
URL=https://github.com/crowning-/fortuneblock.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit fortuneblock=${COMMIT} --url fortuneblock=${URL} ../fortuneblock/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fortuneblock=${COMMIT} --url fortuneblock=${URL} ../fortuneblock/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fortuneblock=${COMMIT} --url fortuneblock=${URL} ../fortuneblock/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the fortuneblock git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fortuneblock/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/ftbfun/fortuneblock-detached-sigs.git

BTCPATH=/some/root/path/fortuneblock
SIGPATH=/some/root/path/fortuneblock-detached-sigs

./bin/gbuild --url fortuneblock=${BTCPATH},signature=${SIGPATH} ../fortuneblock/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fortuneblock-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fortuneblock-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fortuneblock-osx-build.assert
```
...
[fortuneblock/gitian.sigs](https://github.com/ftbfun/gitian.sigs/) repository, or if that's not possible to create a pull
request.
