...
static const std::regex ftb_names("^FTB$|^FORTUNEBLOCK$|^wFTB$|^WFTB$|^FTBcoin$|^FTBCOIN$");


bool IsAssetNameValid(std::string name, bool isRoot) {
    if (name.length() < 3 || name.length() > 128) return false;
    if (isRoot)
        return std::regex_match(name, name_root_characters) && !std::regex_match(name, ftb_names);
    else
        return std::regex_match(name, name_sub_characters) && !std::regex_match(name, ftb_names); 
}
