...
const char *const BITCOIN_CONF_FILENAME = "fortuneblock.conf";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager &am, const std::string &arg) {
        ...
        // argument value seen from the command line (so "fortuneblockd -foo=bar
...
fs::path GetDefaultDataDir() {
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FortuneblockCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FortuneblockCore
    // Mac: ~/Library/Application Support/FortuneblockCore
    // Unix: ~/.fortuneblockcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FortuneblockCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FortuneblockCore";
#else
    // Unix
    return pathRet / ".fortuneblockcore";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string &error, bool ignore_invalid_keys) {
    ...
    if (stream.good()) {
        ...
        // Create an empty fortuneblock.conf if it does not exist
...
std::string CopyrightHolders(const std::string &strPrefix, unsigned int nStartYear, unsigned int nEndYear) {
    ...
    // Check for untranslated substitution to make sure Fortuneblock Core copyright is not removed by accident
    if (copyright_info.find("Fortuneblock") == std::string::npos) {
