...
bool CScript::IsAssetScript(int &nIndex) const {
    if (this->size() > 31) {
        if ((*this)[25] == OP_ASSET_ID) { // OP_FTB_ASSET is always in the 25 index of the script if it exists
            nIndex = -1;
            if ((*this)[27] == FTB_R) { // Check to see if FTB starts at 27 ( this->size() < 105)
                if ((*this)[28] == FTB_T)
                    if ((*this)[29] == FTB_M)
                        nIndex = 30;
            } else {
                if ((*this)[28] == FTB_R) // Check to see if FTB starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == FTB_T)
                        if ((*this)[30] == FTB_M)
                            nIndex = 31;
            }
