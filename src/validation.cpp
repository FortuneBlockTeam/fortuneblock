...
#if defined(NDEBUG)
# error "Fortuneblock Core cannot be compiled without assertions."
#endif

#define MICRO 0.000001
#define MILLI 0.001

const std::string strDefaultFortuneAddress = "fYFrirwfyAbA2YEEYZN9dJXzidfvQ9C2ar";
...
static bool AcceptToMemoryPoolWorker(const CChainParams &chainparams, CTxMemPool &pool, CValidationState &state,
    ...
    if (llmq::quorumInstantSendManager->IsWaitingForTx(hash)) {
        ...
        for (const CTxIn &txin: tx.vin) {
            ...
            if (ptxConflicting) {
                // Transaction conflicts with mempool and RBF doesn't exist in Fortuneblock
...
static int64_t nTimeFortuneblockSpecific = 0;
...
bool CChainState::ConnectBlock(const CBlock &block, CValidationState &state, CBlockIndex *pindex,
    ...
    /// FORTUNEBLOCK: Check superblock start
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, assetsCache, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(FORTUNEBLOCK): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // FORTUNEBLOCK : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto &tx: block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(FORTUNEBLOCK): transaction %s conflicts with transaction lock %s",
    ...
    // FORTUNEBLOCK : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(FORTUNEBLOCK): couldn't find smartnode or superblock payments"),
    ...
    nTimeFortuneblockSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Fortuneblock specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4),
             nTimeFortuneblockSpecific * MICRO, nTimeFortuneblockSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex *pindex, CCoinsViewCache &inputs, const CChainParams &params,
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, assetsCache, false /*fJustCheck*/,
                                  false /*fScriptChecks*/)) {
        return error("RollforwardBlock(FTB): ProcessSpecialTxsInBlock for block %s failed with %s",
                     pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
