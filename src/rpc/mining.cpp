...
// To continue using `generate` in v0.17, restart fortuneblockd with the
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    RPCHelpMan{"generatetoaddress",
        "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
        {
            ...
            {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated FTB to."},
            {"maxtries", RPCArg::Type::NUM, /* default */ "1000000", "How many iterations to try."},
        },
        RPCResult{
            RPCResult::Type::ARR, "", "hashes of blocks generated",
        ...
        RPCExamples{
            ...
            + "If you are running the Fortuneblock Core wallet, you can get a new address to send the newly generated coins to with:\n"
...
static UniValue getblocktemplate(const JSONRPCRequest &request) {
    ...
    if (::ChainstateActive().IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Fortuneblock Core is downloading blocks...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (AreSuperblocksEnabled()
        && !smartnodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(::ChainActive().Height() + 1))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Fortuneblock Core is syncing with network...");
    ...
    UniValue fortuneObj(UniValue::VOBJ);
    FortunePayment fortunePayment = Params().GetConsensus().nFortunePayment;
    if (pblock->txoutFortune != CTxOut()) {
        CTxDestination fortune_addr;
        ExtractDestination(pblock->txoutFortune.scriptPubKey, fortune_addr);
        fortuneObj.pushKV("payee", EncodeDestination(fortune_addr).c_str());
        fortuneObj.pushKV("script", HexStr(pblock->txoutFortune.scriptPubKey));
        fortuneObj.pushKV("amount", pblock->txoutFortune.nValue);
    }
    result.pushKV("fortune", fortuneObj);
    result.pushKV("fortune_payments_started", pindexPrev->nHeight + 1 > fortunePayment.getStartBlock());
...
UniValue setgenerate(const JSONRPCRequest &request) {
    ...
    int numCores = GenerateFortuneblocks(fGenerate, nGenProcLimit, Params(), node);
