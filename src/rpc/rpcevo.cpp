    ...
    static const std::map <std::string, RPCArg> mapParamHelp = {
            {"collateralAddress",
                    {"collateralAddress",       RPCArg::Type::STR,                RPCArg::Optional::NO,
                            "The Fortuneblock address to send the collateral to."}
            },
            {"collateralAmount",
            ...
            {"ownerAddress",
                    {"ownerAddress",            RPCArg::Type::STR,                RPCArg::Optional::NO,
                            "The fortuneblock address to use for payee updates and proposal voting.\n"
            ...
            {"payoutAddress_register",
                    {"payoutAddress_register",  RPCArg::Type::STR,                RPCArg::Optional::NO,
                            "The fortuneblock address to use for smartnode reward payments."}
            },
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Fortuneblock address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Fortuneblock address: ") + request.params[4].get_str());
    } else {
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (!request.params[3].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Fortuneblock address: ") + request.params[3].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceDest);
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::STR_HEX, "tx", "The raw transaction hex of this protx without signiature."},
                      ...
                      {RPCResult::Type::STR, "fortuneblock.conf", "The content of fortuneblock.conf to be used in vps node."},
            }
        },
        RPCExamples{
...
UniValue createConfigFile(std::string blsPrivateKey, std::string ip, std::string address) {
    ...
    std::string fileName = get_current_dir() + "/" + address + "_fortuneblock.conf";
...
//register_prepare "collateralHash" collateralIndex "ipAndPort" "ownerAddress" "operatorPubKey" "votingAddress" operatorReward "payoutAddress" ( "feeSourceAddress" )
    ...
    result.pushKV("fortuneblock.conf",config.get_str());
