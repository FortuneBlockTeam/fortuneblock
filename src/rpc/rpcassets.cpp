...
UniValue sendasset(const JSONRPCRequest &request) {
    if (request.fHelp || !Updates().IsAssetsActive(::ChainActive().Tip()) || request.params.size() < 3 ||
        ...
        throw std::runtime_error(
                ...
                "4. \"change_address\"          (string, optional, default = \"\") the transactions FTB change will be sent to this address\n"
    ...
    if (curBalance == 0) {
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS,
                           "Error: This wallet doesn't contain any FTB, transfering an asset requires a network fee");
    }
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Fortuneblock address: ") + to_address);
    }
    ...
    if (!change_address.empty() && !IsValidDestination(change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           std::string("FTB change address must be a valid address. Invalid address: ") +
                           change_address);
...
UniValue listunspentassets(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of fortuneblock addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) fortuneblock address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"address\" : \"address\",    (string) the fortuneblock address\n"
            "    \"label\" : \"label\",        (string) The associated label, or \"\" for the default label\n"
            "    \"account\" : \"account\",    (string) DEPRECATED. This field will be removed in V0.18. To see this deprecated field, start fortuneblockd with -deprecatedrpc=accounts. The associated account, or \"\" for the default account\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Fortuneblock address: ") + input.get_str());
            }
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !Updates().IsAssetsActive(::ChainActive().Tip()) || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a fortuneblock address\n"
    ...
    if (!IsValidDestination(destination)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Fortuneblock address: ") + address);
    }
